...
# Add arguments for validation of teacher model trained with single token labeling
...
def main():
    ...
    if not args.imagenet_c:
        ...
        average_acc = {}
        base_dir = args.data
        #args.data = '../ImageNet/val/'
        args.data = '../../raid/imagenet_raw/val/'
        res = validate(args)
        print('clean image accuracy is: ', res)
        np.save(results_file + "clean_accuracy.npy", res)
        ...
        np.savetxt(results_file + 'mCE' + '_' + '%.4f' % (mCE/counter) + '.csv', results)
