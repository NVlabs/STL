...
class ToNumpy:
    ...
    def __call__(self, pil_img_teacher, pil_img):
        np_img_teacher = np.array(pil_img_teacher, dtype=np.uint8)
        if np_img_teacher.ndim < 3:
            np_img_teacher = np.expand_dims(np_img_teacher, axis=-1)
        np_img_teacher = np.rollaxis(np_img_teacher, 2)  # HWC to CHW   
        
        np_img = np.array(pil_img, dtype=np.uint8)
        if np_img.ndim < 3:
            np_img = np.expand_dims(np_img, axis=-1)
        np_img = np.rollaxis(np_img, 2)  # HWC to CHW
        return np_img_teacher, np_img
...
class ToTensor:
    ...
    def __call__(self, pil_img_teacher, pil_img):
        ...
        if np_img.ndim < 3:
            np_img = np.expand_dims(np_img, axis=-1)
        np_img = np.rollaxis(np_img, 2)  # HWC to CHW
        return torch.from_numpy(np_img_teacher).to(dtype=self.dtype), torch.from_numpy(np_img).to(dtype=self.dtype)
...
class RandomResizedCropAndInterpolation:
    ...
    def __call__(self, clean_img, img):
        """
        Args:
        ...
        return F.resized_crop(clean_img, i, j, h, w, self.size, interpolation), F.resized_crop(img, i, j, h, w, self.size, interpolation)
